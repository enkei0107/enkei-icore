  ...
  execute() {
    try {
      ...
      if (!fs.existsSync(packageJsonPath)) {
        console.error(`Error: Unable to find package.json in ${privatePackagePath}`);
        process.exit(1);
      }
      ...
      if (packageJson.peerDependencies) {
        ...
        const dependenciesToInstall = Object.entries(peerDependencies).map(
          ([packageName, version]) => `${packageName}@${version}`
        );

        if (dependenciesToInstall.length > 0) {
          console.log(`Installing dependencies: ${dependenciesToInstall.join(", ")}`);

          // Install dependencies using pnpm
          execSync(`pnpm install ${dependenciesToInstall.join(" ")}`, {
            stdio: "inherit",
          });
